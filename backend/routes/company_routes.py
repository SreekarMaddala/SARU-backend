...
# JSON-based login for clients sending { email, password }
@router.post("/login-json", response_model=Token)
def login_for_access_token_json(credentials: CompanyLogin, db: Session = Depends(get_db)):
    ...
    if company:
        ...
        return {"access_token": access_token, "token_type": "bearer"}
    elif credentials.email == ADMIN_EMAIL and credentials.password == ADMIN_PASSWORD:
        access_token_expires = timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
        access_token = create_access_token(
